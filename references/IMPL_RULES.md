...
### R-9: 検証ループ (Verification Loop) — DoD に到達する手順

R-8 が「完了とは何か（状態）」の定義なら、R-9 は「そこへどう到達するか（周期）」の定義です。実装エージェントは「実装した→たぶん通る」で `done` を主張してはいけません（＝検証なき完了の誤報）。**決定的検証（lint / type-check / 該当テスト）を毎周回で実行し、green になるまで回す**こと。これはループエンジニアリングの鉄則「検証＝報酬信号」をチケット粒度に落としたものです。

各チケットは次の周期で処理する:

```
チケットを claim (owner=self, status=in_progress)
起動時に docs/_impl_state/lessons.md を Read（過去の失敗教訓を先取り。R-9.4）
loop (最大 N 回。既定 N=3。orchestrator 指定があれば従う):
    1. 実装 or 直前の失敗ログに基づく修正を書く
    2. 決定的検証を実行する（これが報酬信号。自己申告ではない）:
         - lint        例: ruff / eslint
         - type-check  例: mypy --strict / tsc --noEmit
         - 該当テスト  例: pytest <path> / vitest run <path>
         - (frontend は加えて build とモック視覚比較)
    3. 全 green か?
         yes → evidence に「最終コマンドと出力・試行回数」を貼り status=done で抜ける
         no  → 失敗ログを読む。§4.4 の無進捗検知に当たるなら break
回りきれない時（N 回超過 / 無進捗 / 致命エラー）→ §4.4 に従い orchestrator へエスカレーション。
                                                  コードを書き続けない。失敗教訓を lessons.md に1行残す（R-9.4）。
```

- **R-9.1 決定的検証を judge より優先する** — 機械が pass/fail を返せるもの（テスト・型・lint・build）は必ず回す。LLM の自己評価で代用しない。決定的検証が存在しない部分（設計意図との一致など）だけ人間/レビュアー判断に委ねる。
- **R-9.2 検証を弱めて緑にしない（報酬ハック禁止）** — テストの削除・`skip`/`xfail`/`it.only` の付与・アサーション削除で green を作るのは、目的（仕様の充足）を捨てて検証可能な代理指標だけを満たす行為で、最悪の失敗。`verification_guard.py` hook がこれを検出する想定だが、エージェント側でも自衛する。テストが仕様と食い違って落ちる場合は、勝手に弱めず spec_gaps.md へ。
- **R-9.3 evidence は検証ループの結果を含む** — 何回回ったか・最終 green ログ・（あれば）回復に使った教訓。evidence はレビュアーと orchestrator が信頼する唯一の根拠。
- **R-9.4 失敗から学ぶ（走行内 Reflexion）** — ループ内で「失敗 → 原因特定 → 回復」した場合、`docs/_impl_state/lessons.md` に教訓を1行追記する（例:「T-012: テスト赤。原因=import パス誤り。回避=schemas は src/schemas から import」）。起動時に同ファイルを Read することで、同一走行中の他チケットが同じ轍を踏まない。肥大化防止のため上限内に保つ（§4.2）。
...
├─ lessons.md                  # 走行内エピソード記憶(Reflexion)。検証ループの失敗→回復教訓を1行で蓄積し、各 engineer が起動時に Read する(R-9.4)
└─ review_findings.md          # security-reviewer / code-reviewer の指摘集約
```

> `lessons.md` は **同一案件の走行中だけ効く速い学習ループ**（テンプレ: `${CLAUDE_PLUGIN_ROOT}/templates/_impl_state_lessons_template.md`）。retrospective が回す「案件をまたぐ遅い学習ループ」とは別物で、両者で二層になる。肥大化はコンテキスト腐敗を招くため**直近20件 or 1画面を上限**とし、超過分は案件完了時に retrospective が「IMPL_RULES への恒久ルール昇格」か「破棄」を判断する（loop engineering の compact に相当）。
...
### 4.4 ループの終了・無進捗・予算ガード（ループ規律）

ループエンジニアリングの教え「終了条件はループ設計の半分である」をこのチームの共通語彙にする。R-9（検証ループ）も、orchestrator のチケット再投入も、すべてこの規律の上で回る。**ループは止め方を決めて初めてループになる**。止め方のないループは、無進捗のまま延々とトークンを焼く（このプラグイン最大のコスト事故）。

- **G-1 反復上限（ハードキャップ）** — どのループにも明示的な上限を持たせる。既定値: 検証ループ `N=3`（R-9）、設計の critic↔designer 往復 `M=2`（spec-orchestrator 原則3）。ユーザーが着手時に上書きできる。
- **G-2 無進捗検知** — 直近の試行で「状態が変わらない／同一エラーが連続する」なら、上限前でも break する。**同一チケットで3連続失敗**、または**直前と実質同じ失敗ログ**が出たら無進捗とみなす。回し続けるのではなくエスカレーションする。
- **G-3 予算ガード** — orchestrator は着手時にユーザーへ「最大並列度・コミット粒度」に加えて**想定試行回数の上限**を確認し、超過したらマイルストーンを止めて報告する（黙って焼き続けない）。
- **G-4 回復可能エラーと致命エラーの区別** — ループを続けてよいか即止めるべきかを取り違えない:

  | 種類 | 例 | 対応 |
  |---|---|---|
  | 回復可能（行動材料） | テスト赤・型エラー・lint 違反・ビルド失敗 | ループ内で修正して再検証（R-9） |
  | 致命（即停止） | シークレット/環境変数の欠落、依存解決不能、仕様の根本矛盾、認可設計の不在 | ループを止めて orchestrator → 人間へエスカレーション。spec_gaps.md / incidents.md に記録 |

- **G-5 検証なき完了の禁止（再掲・最重要）** — 「たぶん通る」での `done` は無効。決定的検証の green ログが evidence に無いチケットは未完了として扱う（R-9.3）。報酬ハック（検証を弱めて緑化）も致命扱い（R-9.2 / §6）。
- **G-6 エスカレーション＞推測続行** — 上限到達・無進捗・致命のいずれでも、コードを書き続けるのではなく orchestrator に返す。orchestrator はそれを番号付き質問で人間に上げる。**詰まったループを人間に渡すのは失敗ではなく正しい終了**。

> この規律は impl-orchestrator が采配で適用し、各 engineer が自分のループ（R-9）で守る。設計フェーズ側の対応物は SPEC_RULES と spec-orchestrator 原則3 にある。
...
| `verification_guard.py` | PreToolUse(Write/Edit) | `tests/` のテスト削除・`skip`/`xfail`/`it.only` 付与・アサーション削除を検出（検証ループの報酬ハック防止。R-9.2） |
...
- ❌ **検証を通すためにテストを弱める/消す**（削除・`skip`/`xfail`/`it.only`・アサーション削除）= 報酬ハック。green の代わりに仕様充足を捨てている（R-9.2 / §4.4 G-5）
- ❌ **決定的検証の green ログがないのに `done` にする**（検証なき完了の誤報。R-9.3 / §4.4 G-5）
- ❌ **無進捗のループを止めずに回し続ける**（同一エラー連続・3連続失敗で止めずトークンを焼く。§4.4 G-2）
